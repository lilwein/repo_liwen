...
// #define SERVER_SERIAL 1
// #define SERVER_MPROC 1
// #define SERVER_MTHREAD 1
	...
	**/
	...
	while (1) {
		...
		**/
		...
		if ( recv_bytes == quit_command_len && !memcmp(buf, quit_command, recv_bytes) ){
...
void mprocServer(int server_desc) {
	...
	while (1) {
		...
		pid_t pid = fork();
		if(pid==-1) handle_error("error on fork()");
		if(pid==0){
			ret = close(server_desc);
			if(ret==-1) handle_error("error on close()");
			connection_handler(server_desc, &client_addr);
...
void *thread_connection_handler(void *arg) {
	...
	**/
	...
	int socket_desc = ((handler_args_t *) arg)->socket_desc;
	struct sockaddr_in *client_addr = ((handler_args_t *) arg)->client_addr;
