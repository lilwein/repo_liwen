...
// Searches the node in position i of the list ll, and returns a pointer to its value, 
// or NULL if i is out of bound.
void *linked_list_get(linked_list * ll, int i);

// Removes from the list ll the node in position pos. 
// Notice that the function does not free up the memory pointed by the
...
// Removes the head of the list and returns the node removed. 
// Notice that the function does not free up memory.
...
// Removes all the nodes from the list ll and frees up memory.
// Notice that the function does not free up the memory pointed by the value of the nodes.
